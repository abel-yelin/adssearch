...
async def get_task(task_id: str, task_service: AdsTaskService = Depends(get_ads_task_service)) -> SearchTaskStatusResponse:
    return task_service.get_task_status(task_id)


@router.post("/tasks/{task_id}/cancel", response_model=TaskActionResponse)
async def cancel_task(task_id: str, task_service: AdsTaskService = Depends(get_ads_task_service)) -> TaskActionResponse:
    return task_service.cancel_task(task_id)


@router.post("/tasks/{task_id}/retry", response_model=TaskActionResponse)
async def retry_task(task_id: str, task_service: AdsTaskService = Depends(get_ads_task_service)) -> TaskActionResponse:
